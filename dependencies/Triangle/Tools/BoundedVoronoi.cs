...
namespace TriangleNet.Tools
{
  public class BoundedVoronoi : IVoronoi
  {
    ...
    private bool TriangleIsBlinded(ref Otri tri, ref Osub seg)
    {
      ...
      Point p;
      return this.SegmentsIntersect((Point) vertex4, (Point) vertex5, point, (Point) vertex1, out p, true) || this.SegmentsIntersect((Point) vertex4, (Point) vertex5, point, (Point) vertex2, out p, true) || this.SegmentsIntersect((Point) vertex4, (Point) vertex5, point, (Point) vertex3, out p, true);
    }
    ...
    private void ConstructBoundaryBvdCell(Vertex vertex)
    {
      ...
      do
      {
        ...
        {
          osub.seg = this.subsegMap[o2_1.triangle.hash];
          Vertex vertex3 = osub.SegOrg();
          Vertex vertex4 = osub.SegDest();
          if (!o2_3.triangle.infected)
          {
            vertex2 = o2_1.Dest();
            Vertex vertex5 = o2_1.Apex();
            Point p3 = new Point((vertex2.X + vertex5.X) / 2.0, (vertex2.Y + vertex5.Y) / 2.0);
            if (this.SegmentsIntersect((Point) vertex3, (Point) vertex4, p3, point1, out p, false))
            {
              p.id = count + this.segIndex;
              this.points[count + this.segIndex] = p;
              ++this.segIndex;
            ...
            if (this.SegmentsIntersect((Point) vertex3, (Point) vertex4, point1, point3, out p, true))
            {
              p.id = count + this.segIndex;
              this.points[count + this.segIndex] = p;
              ++this.segIndex;
          ...
          else
          {
            ...
            else
            {
              ...
              if (this.SegmentsIntersect((Point) vertex3, (Point) vertex4, p3, point3, out p, false))
              {
                p.id = count + this.segIndex;
                this.points[count + this.segIndex] = p;
                ++this.segIndex;
      ...
      bool strictIntersect)
    {
      ...
      double num15 = num13;
      if (num11 < 0.0 && num14 < 0.0 || num11 >= 0.0 && num14 >= 0.0 && strictIntersect)
        return false;
