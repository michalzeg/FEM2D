...
namespace FEM2DCommon.Sections
{
    public class SectionPropertiesCalculator
    {
        ...
        private void CalculateExtrimesInPrincipal(IEnumerable<Perimeter> perimeters, double alfa)
        {
            ...
            result.DXI_max = Math.Abs(perimeters.Max(s => s.Coordinates.Max(point => point.X * cos - point.Y * sin)) - xo);
            result.DXI_min = Math.Abs(perimeters.Min(s => s.Coordinates.Min(point => point.X * cos - point.Y * sin)) - xo);
            result.DYI_max = Math.Abs(perimeters.Max(s => s.Coordinates.Max(point => point.X * sin + point.Y * cos)) - yo);
            result.DYI_min = Math.Abs(perimeters.Min(s => s.Coordinates.Min(point => point.X * sin + point.Y * cos)) - yo);
        }

        private void CalculateExtrimesInCentral(IEnumerable<Perimeter> perimeters)
        {
            result.DX0_max = Math.Abs(perimeters.Max(s => s.Coordinates.Max(point => point.X)) - result.X0);
            result.DX0_min = Math.Abs(perimeters.Min(s => s.Coordinates.Min(point => point.X)) - result.X0);
            result.DY0_max = Math.Abs(perimeters.Max(s => s.Coordinates.Max(point => point.Y)) - result.Y0);
            result.DY0_min = Math.Abs(perimeters.Min(s => s.Coordinates.Min(point => point.Y)) - result.Y0);
        }
